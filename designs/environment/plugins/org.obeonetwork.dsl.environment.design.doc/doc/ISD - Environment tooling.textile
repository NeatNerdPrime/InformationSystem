...
h2(#external_links). Lier des éléments d'autres projets de modélisation

De manière générale, la gestion des dépendances entre librairies de modélisation doit se faire via la fonctionnalité dédiée, décrite au chapitre "ISD - Using modeling projects as libraries":../../org.obeonetwork.tools.projectlibrary.doc/doc/ISD%20-%20Using%20modeling%20projects%20as%20libraries.html.

Bien que cette pratique ne soit pas le mode d’utilisation recommandé, il est néanmoins possible d’utiliser les capacités du socle Eclipse Modeling Framework permettant de créer des liens entre projets.

Pour ce faire, il est nécessaire de déclarer la dépendance depuis le projet racine via l’action "Add Model" du menu contextuel des dépendances du projet :

!./pics/ExtLink_AddModelMenu.png!

Puis de sélectionner la ressource du workspace à lier :

!./pics/ExtLink_SelectResourceDialog.png!

Une fois la dépendance déclarée, les outils de la palette de type "Add External Element" peuvent être utilisés pour ajouter des éléments du projet en dépendance :

!./pics/ExtLink_AddExternalElementTool.png!

Pour peupler manuellement une vue partielle avec des éléments externes au projet courant par glisser-déposer depuis l’explorateur de modèles, il est nécessaire de sélectionner les éléments sous le nœud "Project Dependencies" du projet courant :

!./pics/ExtLink_DropOnPartialView.png!

Un glisser-déposer effectué directement depuis le projet lié ne permet pas de peupler le diagramme.
